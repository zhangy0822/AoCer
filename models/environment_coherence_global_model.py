...
class EnvCoherenceGlobalModel(nn.Module):
    ...
    def prepare_feature_logits(self, config):
        ...
        if config.vg_img_logits is not None:
    ...
    def step(self, action1, action2, split):
        ...
        if len(txt_object_index) or len(txt_attribute_index):
            if len(txt_object_index) > 0  and len(txt_attribute_index) == 0:
                glove_vector = torch.mean(self.glove_word_vector[txt_object_index].detach(), dim=0, keepdim=True)
            elif len(txt_object_index) and len(txt_attribute_index):
                glove_vector = torch.mean(self.glove_word_vector[txt_object_index].detach(), dim=0, keepdim=True)
                attribute_glove_vector = torch.mean(self.glove_word_attribute_vector[txt_attribute_index].detach(), dim=0, keepdim=True)
                ...
                glove_vector = torch.mean(self.glove_word_attribute_vector[txt_attribute_index].detach(), dim=0, keepdim=True)
        else:
