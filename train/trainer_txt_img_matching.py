...
class TextImageMatchingTrainer(object):
    ...
    def train(self, train_db, val_db, test_db):
        ...
        for epoch in range(self.epoch, self.cfg.n_epochs):
            ...
            self.adjust_learning_rate(self.cfg, self.optimizer, epoch)
            train_losses = self.train_epoch(train_loaddb, train_loader, epoch)

            val_losses, ri = self.validate_epoch(val_loaddb, val_loader, epoch)

            current_val_loss = np.mean(val_losses)

            is_best = ri > best_ri
            best_ri = max(ri, best_ri)
            if is_best:
                best_epoch = epoch
            logging.info(
                'Time: {:.3f}, Epoch: {}, learning_rate: {}, TrainAverageLoss: {:.3f}, ValAverageLoss: {:.3f}, best_epoch: {}'.format(
                    time() - start, epoch, self.optimizer.param_groups[0]['lr'],
                    np.mean(train_losses),
                    current_val_loss,
                    best_epoch))
            self.save_checkpoint(epoch, self.cfg.exp_name, is_best)
    ...
    def test(self, test_db):
        ...
        ## Evaluation
        ##################################################################
        result = self.evaluate(all_img_feats.cpu().numpy(), all_txt_feats)
    ...
    def train_epoch(self, train_db, train_loader, epoch):
        ...
        for cnt, batched in enumerate(train_loader):
            ...
            if self.cfg.grad_norm_clipping > 0:
                clip_grad_norm_(self.net.parameters(), self.cfg.grad_norm_clipping)

            self.optimizer.step()
            if cnt % self.cfg.log_per_steps == 0:
                logging.info('Epoch: {}, Lr: {}, Iter: {}, Loss: {:.3f}'.format(epoch, self.optimizer.param_groups[0]['lr'], cnt, loss.item()))
        return losses

    def validate_epoch(self, val_db, val_loader, epoch):
        all_img_feats, all_txt_feats, all_sent_msks, losses = [], [], [], []
        self.net.eval()
        for cnt, batched in enumerate(val_loader):
            ...
            with torch.no_grad():
                img_feats, txt_feats = self.net(sent_inds, sent_msks, region_feats)
                batch_loss = self.net.triplet_loss(img_feats, txt_feats, sent_msks)
                loss = torch.mean(batch_loss)
            losses.append(loss.cpu().item())
            all_img_feats.append(img_feats)
            all_txt_feats.append(txt_feats)
            all_sent_msks.append(sent_msks)
            if cnt % self.cfg.log_per_steps == 0:
        ...
        ## Evaluation
        ##################################################################
        ri = self.evaluate(all_img_feats.cpu().numpy(), all_txt_feats.cpu().numpy())

        return losses, ri

    def evaluate(self, all_img_feats, all_txt_feats, all_sent_msks):
        start = time()
        sims = self.shard_sim(all_img_feats, all_txt_feats, all_sent_msks)
        end = time()
        logging.info('compute similarity time: {:.3f}'.format(end - start))
        if self.cfg.use_ensemble:
            return sims, all_img_feats.shape[0]
    ...
    def evaluate_ensemble(self, sims, data_len):
        num_sample = data_len
        r, rt = self.i2t(num_sample, sims)
        ri, rti = self.t2i(num_sample, sims)
        ar = (r[0] + r[1] + r[2]) / 3
    ...
    def shard_sim(self, all_img_feats, all_txt_feats, all_sent_msks, shard_size=128):
        n_img_shard = (all_img_feats.shape[0] - 1) // shard_size
        n_txt_shard = (all_txt_feats.shape[0] - 1) // shard_size
        d = np.zeros((all_img_feats.shape[0], all_img_feats.shape[0]))
        ...
        for i in range(n_img_shard):
            ...
            for j in range(n_txt_shard):
                ...
                txt_start, txt_end = shard_size * j, min(shard_size * (j + 1), all_img_feats.shape[0])
                with torch.no_grad():
                    ...
                    sent_inds = all_sent_msks[j]
                    sim = self.net.compute_sim(img_feat, txt_feat, sent_inds)
                    d[im_start:im_end, txt_start:txt_end] = sim.cpu().numpy()
        return d
    ...
    def save_checkpoint(self, epoch, model_name, is_best):
        ...
        if is_best:
            torch.save(states, osp.join(self.cfg.model_dir, 'best_ri.pkl'))

    def adjust_learning_rate(self, opt, optimizer, epoch):
