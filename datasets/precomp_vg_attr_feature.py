...
def main(config):
    ...
    trainer.net.eval()
    attributes = sorted(train_db.attribute_to_ind.keys(), key=lambda k: train_db.attribute_to_ind[k])
    attributes_idx = [train_db.attribute_to_ind[k] for k in attributes]
    ...
    attr_feat = np.zeros((config.n_property, config.n_feature_dim))
    for key in id2attr.keys():
        attrs = np.array(id2attr[key])
        attr = np.mean(attrs, axis=0)
        attr_feat[key] = attr
    np.save(osp.join(config.data_dir, 'caches/vg_attribute_feat.npy'), attr_feat)
